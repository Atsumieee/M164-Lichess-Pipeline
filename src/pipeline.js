...
import { ensureSchema, mergeImportData } from "./database.js";
...
async function runImport(tournamentIds) {
    ...
    ensureSchema();    // create DB/tables if missing, keep existing data
    mergeImportData(tournaments, [...players.values()], games, standings);   // BULK INSERT into staging, then merge (no duplicates)
